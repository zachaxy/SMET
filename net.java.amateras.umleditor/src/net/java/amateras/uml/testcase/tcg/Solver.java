...
import net.java.amateras.uml.testcase.model.VarRType;
...
public class Solver {
    ...
    public void setVariables(Collection<VarR> vars) {
        for (VarR v : vars) {
            //System.out.println(v.getName()+"->"+v.isInput()+"->"+v.variable+"->"+v.constraint);
            if (!v.isInput()) {
                //System.out.println(v.getName());
                continue;
            }
            variables.put(v.getName(), v.variable);
            if (v.constraint!=null){
                constraints.put(v.getName(),v.constraint);
            }
            //m.addConstraint(v.constraint);
            /*if (v.getType() == VarRType.BOOLEAN) {
               variables.put(v.getName(), Choco.makeBooleanVar(v.getName()));
            } else if (v.getType() == VarRType.DOUBLE) {
            ...
            } else if (v.getType() == VarRType.INT8 || v.getType() == VarRType.UINT8 || v.getType() == VarRType.INT16) {
                int inf = -32768;
                int sup = 32767;
                if (v.getType() == VarRType.INT8) {
                    inf = -128;
                    sup = 127;
                } else if (v.getType() == VarRType.UINT8) {
                    inf = 0;
                    sup = 255;
                }
    ...
    public void setConstraint(String s) {
        System.out.println("要解析的constraint:"+s);
    ...
    public HashMap<String, String> solve() {
        ...
        //solver.setRandomSelectors(System.currentTimeMillis());
        solver.solve();
