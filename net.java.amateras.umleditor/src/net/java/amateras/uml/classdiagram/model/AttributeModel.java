...
public class AttributeModel extends AbstractUMLModel implements Cloneable {
	...
	public void setPropertyValue(Object id, Object value) {
		if (id.equals(P_NAME)) {
			if (!validateInput((String) value)) {
				...
				MessageDialog.openError(Display.getCurrent().getActiveShell(), "变量名格式错误",
			...
			if (!validateCons(this.getType(), (String) value)) {
				MessageDialog.openError(Display.getCurrent().getActiveShell(), "约束条件有误", "约束条件语法不合法！");
				return;
			}
	...
	public boolean validateCons(String type, String cons) {
		String[] qujian = cons.split("\\|\\|");
		String tc = cons.trim();
		if(cons.startsWith("||") || cons.endsWith("||"))
			return false;
		boolean b = true;
		int i;
		for (i = 0; i < qujian.length && b; i++) {
			String s = qujian[i].trim();
			if (type.equals("Int"))
				b = validInt(s);
